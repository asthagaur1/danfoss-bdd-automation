...
function EJStemplates() {
        ...
        for (var i in tests){\
            if(typeof tests[i].tests !== 'undefined'){\
                ...
                && (typeof tests[i].last==='undefined' || tests[i].last==true  ) )\
                    fcount += 1;\
                else if (((typeof tests[i].isSkipped !== 'undefined')&&(tests[i].isSkipped))||(tests[i].status == EJStemplates.statusEnum.SKIPPED))  \
                    scount += 1;\
            }\
        }\
    }\
    ...
    var procNum = 100;\
    if(subElem>0){\
        procNum = Math.floor((pcount/(subElem-scount))*100);\
    } \
    if ((status==EJStemplates.statusEnum.SKIPPED)||(result=='FATAL')||(result=='ERROR'))\
        procNum = 0;\
...
<table class='ui celled table'>\
  ...
  <tbody>\
    ...
    <tr class='disabled'>\
      ...
      <td id='disSc'><%= count(tests,'scenario',EJStemplates.statusEnum.SKIPPED) + count(tests,'row',EJStemplates.statusEnum.SKIPPED) %></td>\
