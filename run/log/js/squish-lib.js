...
// Calculates status (PASS,FAIL,WARNING,SKIPPED) of all tests elements
function setStatus(tests) {
    var status = statusEnum.PASS;
    for (var i in tests) {

        // Test element does NOT include subelements
        // This might also apply to skipped Test Cases
        if (typeof tests[i].tests === 'undefined') {
            if (tests[i].type == "msg") {
                ...
                tests[i].tests = [];
            } else if (tests[i].type == "uicoverage") tests[i].status = statusEnum.PASS;
        }
        ...
        if ((typeof tests[i].last !== 'undefined') && (tests[i].last == false)) {
            ...
            status = statusEnum.PASS
        else if (tests[i].status == statusEnum.SKIPPED && i == 0)
...
function renderContent(tests, prefix) {
    ...
    for (var i in tests) {
        ...
        else if (tests[i].status == statusEnum.SKIPPED)
...
function setOverallStatus(results) {
    ...
    for (var i in results.tests) {
        if (results.tests[i].status == statusEnum.FAIL)
            ...
            status = statusEnum.WARNING
        else if (results.tests[i].status == statusEnum.PASS && status < statusEnum.PASS)
            status = statusEnum.PASS
        else if (results.tests[i].status == statusEnum.PASS && status < statusEnum.PASS)
            status = statusEnum.PASS
        else if (results.tests[i].status == statusEnum.SKIPPED && i == 0)
            status = statusEnum.SKIPPED
    }
